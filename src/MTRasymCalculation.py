...
def correct_B0(emr_data, B0_shift_map):
    M0 = emr_data[0] # [15, 256, 256]
...
def set_geometry_EMR(spacing, origin, direction, img):
    """
    input: EMR geometry (4D), img (3D)
    need to use 3D
    """
    spacing = spacing[0:3]
    origin = origin[0:3]
...
def cal_MTRasym(processed_cases_dir, patient_name, view, seq_dict):
    mapping_dir = os.path.join(processed_cases_dir, patient_name+"_all", view)
    ...
    if emr_1p5uT_id > 0:
        emr_1p5uT, spacing, origin, direction = read_EMR(mapping_dir, emr_1p5uT_id)
        img_arr = correct_B0(emr_1p5uT, B0_shift_map) 
        img = sitk.GetImageFromArray(img_arr)
        img = set_geometry_EMR(spacing, origin, direction, img)
        B0_shift_img = set_geometry_EMR(spacing, origin, direction, B0_shift_img)
        sitk.WriteImage(img, os.path.join(mapping_dir, "APTw.nii")) 
        sitk.WriteImage(B0_shift_img, os.path.join(mapping_dir, "B0_shift_map.nii")) 
    if emr_2uT_id > 0:
        ...
        img = sitk.GetImageFromArray(img_arr)
        sitk.WriteImage(img, os.path.join(mapping_dir, patient_name+"_"+str(emr_2uT_id)+"_apt.nii")) 
    return

def get_M0(processed_cases_dir, patient_name, view, seq_dict):
    mapping_dir = os.path.join(processed_cases_dir, patient_name+"_all", view)
    emr_1p5uT_id = seq_dict["MTR_1p5uT_id"]
    if emr_1p5uT_id > 0:
        emr_1p5uT, spacing, origin, direction = read_EMR(mapping_dir, emr_1p5uT_id)
        img = sitk.GetImageFromArray(emr_1p5uT[0])
        img = set_geometry_EMR(spacing, origin, direction, img)
        sitk.WriteImage(img, os.path.join(mapping_dir, "M0.nii")) 
    return
...
def set_geometry(spacing, origin, direction, img):
    img.SetSpacing(spacing)
    img.SetOrigin(origin)
    img.SetDirection(direction)
    return img

def strip_helper(mapping_dir, output_dir, skull_arr, img_name):
    print("********", img_name, "********")
    img = sitk.ReadImage(os.path.join(mapping_dir, img_name), sitk.sitkFloat32)
    spacing, origin, direction = get_geometry(img)
    arr = sitk.GetArrayFromImage(img)
    min_ = np.min(arr)
    arr[skull_arr == 0] = min_
    img = sitk.GetImageFromArray(arr)
    img = set_geometry(spacing, origin, direction, img)
    sitk.WriteImage(img, os.path.join(output_dir, img_name))
    return

def strip_skull(processed_cases_dir, patient_name, view):
    mapping_dir = os.path.join(processed_cases_dir, patient_name+"_all", view)
    output_dir = os.path.join(mapping_dir, "stripped")
    os.makedirs(output_dir, exist_ok=True)
    skull = sitk.ReadImage(os.path.join(mapping_dir, "skull.nii.gz"), sitk.sitkInt32)
    skull_arr = sitk.GetArrayFromImage(skull)
    strip_helper(mapping_dir, output_dir, skull_arr, "M0.nii")
    strip_helper(mapping_dir, output_dir, skull_arr, "APTw.nii")
    strip_helper(mapping_dir, output_dir, skull_arr, "T1_map.nii")
    strip_helper(mapping_dir, output_dir, skull_arr, "T2_map.nii")
    return  
